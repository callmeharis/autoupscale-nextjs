...
import { Accordion, Button, Carousel, Col, Container, Form, Modal, Row } from 'react-bootstrap';
...
export default function CustomerHistory({ id }) {
    ...
    useEffectAsync(async () => {
        if (id) {
            try {
                const data = await customerApi.history.getById(id, form.values);
                setReports(data);
            } catch (error) {
    ...
    return (
        <>
            { <div>
                <div className="">
                    <Accordion className="filter-searchbar-accordion text-end w-full">
                        <Accordion.Header className=" inline-block mb-2">

                            <Button className="border text-sm bg-transparent text-black p-[6px] rounded  flex items-center">
